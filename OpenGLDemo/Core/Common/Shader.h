...
{
public:
	static std::string pathPrefix; // Shaders/Chapter5/
	...
	Shader(const char* vertexPath, const char* fragmentPath, const char* geometryPath = 0)
	{
		...
		std::string gpath(geometryPath ? geometryPath : "");
		vpath.insert(0, pathPrefix);
		fpath.insert(0, pathPrefix);
		if (gpath.length() > 0)
			gpath.insert(0, pathPrefix);

		if (!BeginsWith(vpath.c_str(), "Shaders/"))
			vpath.insert(0, "Shaders/");
		if (!BeginsWith(fpath.c_str(), "Shaders/"))
			fpath.insert(0, "Shaders/");
		if (gpath.length() > 0 && !BeginsWith(gpath.c_str(), "Shaders/"))
			gpath.insert(0, "Shaders/");
